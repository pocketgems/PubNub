...
@property (nonatomic, readonly, strong) NSData *httpHeaderStartData;

// Stores reference on data object which is used
// to mark chunked content end in HTTP response
// body
@property (nonatomic, readonly, strong) NSData *httpChunkedContentEndData;

// Stores reference on data object which is used
// to find new line chars (\r\n) in provided data
@property (nonatomic, readonly, strong) NSData *endLineCharactersData;
...
- (id)init {
    ...
    if((self = [super init])) {
        ...
        _httpHeaderStartData = [@"HTTP/1.1 " dataUsingEncoding:NSUTF8StringEncoding];
        _httpChunkedContentEndData = [@"0\r\n\r\n" dataUsingEncoding:NSUTF8StringEncoding];
        _endLineCharactersData = [@"\r\n" dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)parseResponseData:(NSMutableData *)data withBlock:(void (^)(NSArray *responses))parseCompletionBlock {
    ...
    [self pn_dispatchBlock:^{
        @synchronized(data) {
            ...
            NSRange responseRange = NSMakeRange(0, [data pepnSynced_length]);
            NSRange contentRange = NSMakeRange(0, [data pepnSynced_length]);
            ...
            void(^malformedResponseParseErrorHandler)(NSData *, NSRange) = ^(NSData *responseData, NSRange subrange) {
                ...
                [PNLogger logDeserializerErrorMessageFrom:self withParametersFromBlock:^NSArray *{
                    ...
                    NSData *failedResponseData = [responseData pepnSynced_subdataWithRange:subrange];
            ...
            if(contentRange.location != NSNotFound) {
                ...
                [data pepnSynced_setData:[data pepnSynced_subdataWithRange:NSMakeRange(lastResponseEndIndex, [data pepnSynced_length]-lastResponseEndIndex)]];
            }
...
- (PNResponse *)responseInRange:(NSRange)responseRange ofData:(NSData *)data incompleteResponse:(BOOL *)isIncompleteResponse {
    ...
    @synchronized(data) {
        responseSubdata = [data pepnSynced_subdataWithRange:responseRange];
    }
...
- (NSData *)joinedDataFromChunkedDataUsingOctets:(NSData *)chunkedData {
    ...
    @synchronized(chunkedData) {
        BOOL parsingChunkOctet = YES;
        BOOL parsingChunk = NO;
        NSRange searchRange = NSMakeRange(0, [chunkedData pepnSynced_length]);
        NSUInteger chunkStart = searchRange.location;
        
        NSRange cursor = [chunkedData pepnSynced_rangeOfData:self.endLineCharactersData
        ...
        while (cursor.location != NSNotFound) {
            ...
            NSData *chunk = [chunkedData pepnSynced_subdataWithRange:NSMakeRange(chunkStart, cursor.location - chunkStart)];
            ...
            if (parsingChunk) {
                ...
                cursor = [chunkedData pepnSynced_rangeOfData:self.endLineCharactersData
                                                     options:(NSDataSearchOptions)0
    ...
    @synchronized(data) {
        if ([data pepnSynced_length]) {
            ...
            if (searchRange.location != NSNotFound && searchRange.location + searchRange.length <= [data length]) {
                ...
                range = [data pepnSynced_rangeOfData:self.httpHeaderStartData
                                             options:(NSDataSearchOptions)0
                                               range:searchRange];
            }
        }
