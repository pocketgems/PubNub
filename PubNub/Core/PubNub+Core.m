...
DDLogClientInfo([[self class] ddLogLevel], @" PubNub SDK %@ (%@)", kPNLibraryVersion,
kPNCommit);
...
if (result) { DDLogResult([[self class] ddLogLevel], @" %@", [result stringifiedRepresentation]); }

if (status) {

if (status.isError) {

DDLogFailureStatus([[self class] ddLogLevel], @" %@", [status stringifiedRepresentation]);
}
else {

DDLogStatus([[self class] ddLogLevel], @" %@", [status stringifiedRepresentation]);
}
}
...
DDLogClientInfo([[self class] ddLogLevel], @" Did enter background execution context.");
}
else if ([notification.name isEqualToString:NSExtensionHostWillEnterForegroundNotification]) {

DDLogClientInfo([[self class] ddLogLevel], @" Will enter foreground execution context.");
}
#elif __IPHONE_OS_VERSION_MIN_REQUIRED || defined(PGDROID)
...
DDLogClientInfo([[self class] ddLogLevel], @" Workspace became inactive.");
}
...
DDLogClientInfo([[self class] ddLogLevel], @" Workspace became active.");
}
#endif
}
