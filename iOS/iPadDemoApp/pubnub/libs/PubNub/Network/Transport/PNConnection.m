...
typedef NS_OPTIONS(int, PNConnectionActionSourceFlag)  {
...
typedef NS_OPTIONS(int, PNConnectionActionFlag)  {
...
typedef NS_OPTIONS(int, PNConnectionActionOwnerFlag)  {
...
typedef NS_OPTIONS(int, PNConnectionDataSendingStateFlag)  {
...
typedef NS_OPTIONS(int, PNConnectionCleanStateFlag)  {
...
static int const kPNMaximumConfigurationRetryCount = 3;

// Maximum connection retry count which can be performed before library will report connection error
static int const kPNMaximumConnectionRetryCount = 3;
...
@property (nonatomic, assign) int configurationRetryCount;
@property (nonatomic, assign) int connectionRetryCount;
    ...
    if (CFReadStreamHasBytesAvailable(self.socketReadStream)) {
        ...
        if (readedBytesCount > 0) {
            ...
            if ([PNLogger isDumpingHTTPResponse] || [PNLogger isLoggerEnabled]) {
                ...
                NSData *tempData = [NSData dataWithBytes:buffer length:(int)readedBytesCount];
            ...
            if (self.deserializer.isDeserializing) {
                ...
                [self.temporaryRetrievedData appendBytes:buffer length:(int)readedBytesCount];
            }
            else {

                // Store fetched data
                [self.retrievedData appendBytes:buffer length:(int)readedBytesCount];
                [self processResponse];
            }
        }
...
- (void)processResponse {
    ...
    if ([responses count] > 0) {
        ...
        [responses enumerateObjectsUsingBlock:^(id response, int responseIdx, BOOL *responseEnumeratorStop) {
